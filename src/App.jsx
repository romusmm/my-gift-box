...
export default function MyGiftBoxSite() {
  ...
  // =============================
  // Pruebas ligeras (solo en desarrollo)
  // =============================
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (process.env.NODE_ENV === "production") return;
    // Test 1: buildContactMsg compone con saltos de línea
    const msg1 = buildContactMsg({ nombre: "Ana", interes: "KIT2", mensaje: "Hola" });
    console.assert(msg1.includes("\n"), "[TEST] buildContactMsg debe incluir \n entre líneas");
    // Test 2: WhatsApp URL codifica saltos de línea como %0A
    const url = WHATSAPP_LINK_BASE(msg1);
    console.assert(url.includes("%0A"), "[TEST] URL de WhatsApp debe contener %0A (newline codificado)");
    // Test 3: Navegación a detalle de kit
    console.assert(typeof openKit === "function", "[TEST] openKit debe existir");
  }, []);
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <header className="fixed top-0 inset-x-0 z-50">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mt-4 mb-3 flex items-center justify-between rounded-2xl bg-neutral-900/80 backdrop-blur px-3 py-2 border border-white/10">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-white rounded-xl"><Gift className="h-5 w-5 text-neutral-900"/></div>
              <div className="font-semibold text-white">{BRAND}</div>
            </div>
            <nav className="flex items-center gap-1">
              <NavLink label="Inicio" active={page===PAGES.LANDING} onClick={() => setPage(PAGES.LANDING)} />
              <NavLink label="Kits" active={page===PAGES.KITS || page===PAGES.KIT} onClick={() => setPage(PAGES.KITS)} />
              <NavLink label="Cómo funciona" active={page===PAGES.HOW} onClick={() => setPage(PAGES.HOW)} />
              <NavLink label="Historia" active={page===PAGES.ABOUT} onClick={() => setPage(PAGES.ABOUT)} />
              <NavLink label="Contacto" active={page===PAGES.CONTACT} onClick={() => setPage(PAGES.CONTACT)} />
              <MBtn
  whileHover={{ scale: 1.02 }}
  whileTap={{ scale: 0.98 }}
  className="ml-2 px-4 py-2 rounded-lg font-semibold text-white transition-all duration-300 
  ...
  <Phone className="mr-2 h-4 w-4" /> Pedir por WhatsApp
      ...
      <footer className="mt-20 border-t bg-white">
        <Section className="py-10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">© {new Date().getFullYear()} {BRAND}. Hecho con cariño.</div>
        </Section>
      </footer>
    </div>
  );
